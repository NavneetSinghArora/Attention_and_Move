...
class RLAgent(Generic[EnvType]):
    ...
    def gpuify(self, tensor: torch.Tensor) -> torch.Tensor:
        if self.gpu_id >= 0:
            with torch.cuda.device(self.gpu_id):
                tensor = tensor.cuda()
        return tensor
    ...
    def repackage_hidden(self) -> None:
        self.hidden = net.recursively_detach(self.hidden)
    ...
    def preprocess_frame(
        ...
        frame = net.resnet_input_transform(frame, new_frame_size)
