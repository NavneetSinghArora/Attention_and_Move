...
class AI2ThorEnvironment(object):
    ...
    def current_depth_frames(self) -> Tuple[np.ndarray, ...]:
        if not self.render_depth_image:
            raise Exception("Depth frames are not available, must set render_depth_image to true before initializing.")
        return tuple(
            self.controller.last_event.events[i].depth_frame for i in range(self.num_agents)
        )
    ...
    def teleport_agent_to(
        ...
        if only_initially_reachable:
            ...
            if not reachable:
                ...
                self.last_event.metadata["errorMessage"] = "Target position was not initially reachable."
