...
class SaveFurnMoveMixin(object):
    ...
    def data_save_dir(self) -> str:
        return os.path.join(CONSTANTS.ABS_PATH_TO_DATA_DIR, "furnmove_evaluations__{}/{}").format(
    ...
    def create_episode_init_queue(self, mp_module) -> torch.multiprocessing.Queue:
        ...
        with open(
            os.path.join(CONSTANTS.ABS_PATH_TO_DATA_DIR, "{}").format(
...
class SaveFurnLiftMixin(object):
    ...
    def data_save_dir(self) -> str:
        ...
        return os.path.join(
            CONSTANTS.ABS_PATH_TO_DATA_DIR, "furnlift_evaluations__{}/{}__{}"
        ).format(
            self.episode_init_queue_file_name.split("__")[-1].replace(".json", ""),
            self.simple_name(),
            unique_dir,
...
def save_agents_path_without_frame_png(
    ...
    plt.savefig(
        os.path.join(
            CONSTANTS.ABS_PATH_TO_ANALYSIS_RESULTS_DIR, "episode_run_paths/{}_{}.jpg"
