...
from cordialsync.ai2thor.gridworld import AI2ThorLiftedObjectGridEnvironment
...
def lifted_furniture_step(
    ...
    if any(dir in action for dir in CONSTANTS.CARD_DIR_STRS):
        action_dir = None
        for dir in CONSTANTS.CARD_DIR_STRS:
            if dir in action:
                action_dir = dir
...
class MultiAgentMovingWithFurnitureBaseEpisode(MultiAgentAI2ThorEpisode):
    def __init__(
        ...
        failed_action_penalty=CONSTANTS.FAILED_ACTION_PENALTY,
        ...
        **kwargs,
        ...
        self.joint_pass_penalty = CONSTANTS.JOINT_PASS_PENALTY
    ...
    def multi_step(self, actions_as_ints: Tuple[int, ...]) -> List[Dict[str, Any]]:
        ...
        is_joint_move_with = tuple(
            [
                CONSTANTS.JOINT_MOVE_WITH_OBJECT_KEYWORD.lower() in action.lower()
        ...
        is_joint_move_object = tuple(
            [
                CONSTANTS.JOINT_MOVE_OBJECT_KEYWORD.lower() in action.lower()
                for action in actions_as_strings
            ]
        )
        ...
        is_joint_rotate = tuple(
            [
                CONSTANTS.JOINT_ROTATE_KEYWORD.lower() in action.lower()
                for action in actions_as_strings
            ]
        )
    ...
    def coordinated_multi_step(
        ...
        for substr in CONSTANTS.CARD_DIR_STRS + CONSTANTS.EGO_DIR_STRS:
